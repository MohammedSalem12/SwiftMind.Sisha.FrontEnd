    ...
    <div class="page">
      <div class="form-card">
        <div class="card-header-section">
          ...
          <div>
            <h2>تعديل الموعد</h2>
            <p class="subtitle">تعديل تفاصيل موعد الحصة</p>
          </div>
        </div>
        ...
        <form *ngIf="!loading()" class="card-body-section" (ngSubmit)="submit()">
          ...
          <div class="actions">
            <button class="btn-primary" type="submit" [disabled]="saving() || !startTime || !endTime">
...
export class EditScheduleComponent implements OnInit {
  ...
  private async loadSchedule() {
    ...
    try {
      // Load all groups with schedules to find this schedule
      const groups = await lastValueFrom(this.groupService.getList());
      if (groups.items) {
        for (const group of groups.items) {
          // Try to load schedules for this group
          ...
          if (teacherId && courseId) {
            ...
            for (const gwsDto of groupsWithSchedules) {
              const schedule = gwsDto.schedules?.find(s => s.id === this.scheduleId);
              if (schedule) {
  ...
  async submit() {
    if (!this.scheduleId || !this.startTime || !this.endTime) return;
